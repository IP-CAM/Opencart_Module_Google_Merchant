...
class Googleshopping extends Library
{
    ...
    public function __construct($registry, $store_id)
    {
        ...
        if ($this->store_id === 0) {
            ...
            $this->store_url = $this->getSettingValue('config_ssl', $store_id);
            $this->store_name = $this->getSettingValue('config_name', $store_id);
        }
    ...
    public function doJob($job)
    {
        ...
        $this->debugLog("api  000");

        $response = $this->api($init_request);

        $this->debugLog("doJob  002");
    ...
    public function cron()
    {
        ...
        foreach ($this->getJobs() as $job) {
            try {
                ...
                $this->config->set("config_seo_url", $this->getSettingValue("config_seo_url", $this->store_id));

                $report[] = $this->output("DO CRON " . json_encode($job));

                $this->debugLog("doJob  000");

                // Do the CRON job
                $count = $this->doJob($job);

                $report[] = $this->output("DONE CRON " . json_encode($job));
                ...
                $report[] = "ERROR " . $this->output($e->getMessage());
            }
        }
    ...
    private function api($request)
    {
        ...
        $this->debugLog("curl_setopt_array: " . print_r($curl_options, true));
        $this->debugLog("CURLOPT_POSTFIELDS: " . print_r($request['data'], true));
        $this->debugLog("CURLOPT_HTTPHEADER: " . $headers);
        $this->debugLog("RESPONSE: " . $result);
        $this->debugLog("CURL INFO: " . print_r($info, true));
